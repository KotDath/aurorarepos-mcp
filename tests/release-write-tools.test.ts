...
describe('release write user confirmation through MCP', () => {
  ...
  async function connect(mode: 'legacy' | 'auto', response?: ElicitResult, mutate?: (test: Awaited<ReturnType<typeof setupReleaseWrites>>) => void) {
    const test = await setupReleaseWrites(directory), publicFetch = vi.fn(async () => { throw Error('Unexpected public HTTP'); });
    const server = createServer(new AuroraService(new AuroraClient({ fetch: publicFetch })), new AuthService(test.store), undefined, undefined, undefined, test.service);
