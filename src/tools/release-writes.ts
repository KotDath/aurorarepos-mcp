...
export function registerReleaseWriteTools(server: McpServer, service: ReleaseWriteService): void {
  ...
  server.registerTool('update_my_app_version', { title: 'Update owned release metadata',
    description: 'Change plain-text description, category_id and/or release_notes for an owned version_id. Description/category are shared APP fields affecting all releases. Preserves unchanged contacts, owner, icon, screenshots, beta/scheduling and RPM references. Requires exact user confirmation and unchanged state. Does not replace RPMs. Never automatically retry uncertain outcomes.',
  ...
  server.registerTool('schedule_my_app_version', { title: 'Set owned publication schedule',
    description: 'Set/cancel website is_delayed and publish_at for an owned version. Date is YYYY-MM-DDTHH:mm in WEBSITE wall-clock time: server timezone and actual delayed publication not verified. Does not force a status transition or bypass moderation. Preserves other editor fields and RPMs; requires exact user confirmation. Never retry unknown outcomes.',
