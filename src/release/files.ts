...
export async function readPackage(file: string, signal: AbortSignal, capture = false, maxBytes = MAX_FILE_BYTES) {
  checkSignal(signal);
  ...
  try {
    ...
    if (before.size > BigInt(Math.min(MAX_FILE_BYTES, maxBytes))) throw new ReleaseError('FILE_TOO_LARGE');
    ...
    try {
      ...
      const metadata = await readRpm(read, size), hash = createHash('sha256'), chunks: Buffer[] = [];
      for (let position = 0; position < size; position += 64 * 1024) {
        const chunk = await read(position, Math.min(64 * 1024, size - position));
        hash.update(chunk); if (capture) chunks.push(chunk);
      }
